    ...
    $.extend({
        ...
        table: {
            ...
            exportExcel: function(formId) {
                table.set();
                $.modal.confirm("すべてのデータ（" + table.options.modalName + "）をエクスポートするか？", function() {
                    var currentId = $.common.isEmpty(formId) ? $('form').attr('id') : formId;
                    var params = $("#" + table.options.id).bootstrapTable('getOptions');
                    var dataParam = $("#" + currentId).serializeArray();
                    dataParam.push({ "name": "orderByColumn", "value": params.sortName });
                    dataParam.push({ "name": "isAsc", "value": params.sortOrder });
                    $.modal.loading("エクスポート中。。。。。。");
                    $.post(table.options.exportUrl, dataParam, function(result) {
                        if (result.code == web_status.SUCCESS) {
                            alert(ctx + "common/download?fileName=" + encodeURI(result.msg) + "&delete=" + true);
            ...
            importExcel: function(formId) {
                ...
                layer.open({
                    ...
                    btn: ['<i class="fa fa-check"></i> インプット', '<i class="fa fa-remove"></i> キャンセル'],
                    // 弹层外区域关闭
                    shadeClose: true,
                    btn1: function(index, layero){
                        var file = layero.find('#file').val();
                        if (file == '' || (!$.common.endWith(file, '.xls') && !$.common.endWith(file, '.xlsx'))){
                            $.modal.msgWarning("拡張子（xls,xlsx）を選択してください。");
                            return false;
                        }
        ...
        modal: {
            ...
            openOptions: function (options) {
                ...
                var _title = $.common.isEmpty(options.title) ? "システム窓口" : options.title; 
        ...
        operate: {
            ...
            detailUrl: function(id) {
                ...
                if ($.common.isNotEmpty(id)) {
                    ...
                    if (id.length == 0) {
                        $.modal.alertWarning("1個を選択してください");
                        return;
                    }
                    url = table.options.detailUrl.replace("{id}", id);
                }
            ...
            remove: function(id) {
                table.set();
                $.modal.confirm("このデータ（" + table.options.modalName + "）を削除するか？", function() {
            ...
            add: function(id) {
                table.set();
                $.modal.open("追加" + table.options.modalName, $.operate.addUrl(id));
            },
            // 添加信息，以tab页展现
            addTab: function (id) {
                table.set();
                $.modal.openTab("追加" + table.options.modalName, $.operate.addUrl(id));
            },
            // 添加信息 全屏
            addFull: function(id) {
                table.set();
                var url = $.common.isEmpty(id) ? table.options.createUrl : table.options.createUrl.replace("{id}", id);
                $.modal.openFull("追加" + table.options.modalName, url);
            },
            // 添加访问地址
            addUrl: function(id) {
            ...
            edit: function(id) {
                ...
                if($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
                    ...
                    $.modal.open("編集" + table.options.modalName, url);
                } else {
                    $.modal.open("編集" + table.options.modalName, $.operate.editUrl(id));
                }
            },
            // 修改信息，以tab页展现
            editTab: function(id) {
                table.set();
                $.modal.openTab("編集" + table.options.modalName, $.operate.editUrl(id));
            },
            // 修改信息 全屏
            editFull: function(id) {
                ...
                $.modal.openFull("編集" + table.options.modalName, url);
            },
            // 修改访问地址
            editUrl: function(id) {
                var url = "/404.html";
            ...
            // 保存信息 弹出提示框 成功关闭窗口
            saveAndClose: function(url, data, callback) {
                var config = {
                    url: url,
                    type: "post",
                    dataType: "json",
            ...
            saveSuccess: function (result) {
                if (result.code == web_status.SUCCESS) {
                    $.modal.msgReload("保存が完成し、リロード中。。。。。。", modal_status.SUCCESS);
                } else if (result.code == web_status.WARNING) {
                    $.modal.alertWarning(result.msg)
                }  else {
                    $.modal.alertError(result.msg);
